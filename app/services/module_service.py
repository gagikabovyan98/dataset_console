...
def list_modules(
    ...
    *,
    ...
    stmt = stmt.where(
        or_(
            DCModule.is_private.is_(False),
            and_(
                DCModule.is_private.is_(True),
                or_(
                    (user_id is not None) & (DCModule.added_by_user_id == user_id),
                    (usergroup_id is not None) & (DCModule.usergroup_id == usergroup_id),
                ),
            ),
        )
    )
