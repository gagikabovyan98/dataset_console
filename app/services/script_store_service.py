...
def create_script(name: str, code: str, meta: dict | None = None, workspace_name: str = "default") -> int:
    with SessionLocal() as s:
        ...
        script = DCScript(name=name, code=code, meta=meta or {}, workspace_id=ws.id)
...
def list_scripts(limit: int = 100) -> list[dict]:
    with SessionLocal() as s:
        rows = s.scalars(select(DCScript).order_by(DCScript.id.desc()).limit(limit)).all()
        return [
            {
                "id": r.id,
                "name": r.name,
